...
function usage() {
  ...
  console.error('events: screenshot [name], button <name> <on|off>, new-game, warp <group> <map> <x> <y>, battle-transition <id>, avatar <mode>, running-shoes <on|off>, weather <0..15>, view <classic|hd2d>, gpu-loss, probe <name>');
  process.exit(2);
}
...
function parseEvents(text) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (fields[1] === 'battle-transition') {
      const transition = Number(fields[2]);
      if (fields.length !== 3 || !Number.isInteger(transition) || transition < 0)
        throw new Error(`${index + 1}: expected "<frame> battle-transition <id>"`);
      events.push({ frame, type: 'battle-transition', transition });
      continue;
    }
...
async function main() {
  ...
  try {
    ...
    for (const event of events) {
      ...
      } else if (event.type === 'battle-transition') {
        await evaluate(cdp, `window.pokeemerald.automation.startBattleTransition(${event.transition})`);
